...
`dish-maskable.png`, `manifest.json`, `README.md`
...
## Sắp xếp mặc định, nút Ngẫu nhiên, link Chỉ đường

### Sắp xếp mặc định theo thứ tự thêm vào

Ban đầu mặc định sort theo tên (A-Z). Đã đổi lại: mỗi quán được gắn thêm
`__order` (index gốc trong `DATA`, gán 1 lần lúc load qua
`DATA.forEach((d, i) => { d.__order = i; })`), và state khởi tạo
`sortKey: "order", sortDir: -1` — nghĩa là **quán thêm vào sau (nằm cuối
`data.js`) hiện lên đầu danh sách** theo mặc định. Nhấn vào tiêu đề cột
Quán/Loại/Chi nhánh vẫn đổi sang sort theo cột đó như bình thường.

### Nút 🎲 Ngẫu nhiên

Nút riêng (không đụng vào tiêu đề cột), đặt cạnh ô tìm kiếm. Mỗi lần bấm:
gán lại 1 số ngẫu nhiên (`Math.random()`) cho từng quán vào `Map`
`randomWeights`, set `state.sortKey = "random"`, rồi `render()`. Giữ nguyên
thứ tự đó (không tự xáo lại) khi gõ tìm kiếm hay đổi tab lọc, cho tới lần
bấm nút kế tiếp. Style riêng (`.random-btn` trong `style.css`) khớp theme
sổ tay thay vì dùng nút Bootstrap mặc định (`btn-outline-secondary` nhìn
lệch tông xanh dương).

### Link Chỉ đường (Google Maps)

Có tài khoản Google Cloud Console nhưng **không dùng Directions API/Maps
JavaScript API** — vì site tĩnh public trên GitHub Pages, API key nhúng
client-side sẽ lộ trong mã nguồn (giới hạn theo HTTP referrer chỉ hạn chế
domain khác dùng ké, không giấu được key), lại phải bật billing dù có free
credit. Chọn giải pháp không cần key: link deep-link thẳng tới Google Maps

```
https://www.google.com/maps/dir/?api=1&destination=<địa chỉ đã encodeURIComponent>
```

đặt luôn cạnh địa chỉ trong `branch-addr` (ký hiệu 📍, không xuống dòng
riêng). Bấm vào mở Google Maps (web/app) và tự chỉ đường từ vị trí hiện tại
của người xem đến quán.
